...
namespace unicorn {
    ...
    RuntimeValues::RuntimeValues(const char *in_utf_8_json_string)
            : type_(Type::JSONObject) {
        data_.utf_8_str = in_utf_8_json_string;
    }

    RuntimeValues::RuntimeValues(const std::string &in_string) noexcept
            : type_(Type::STRING) {
    ...
    bool RuntimeValues::GetAsUtf8JsonStr(const char **out_value) const {
        if (IsJsonObject()) {
            *out_value = data_.utf_8_str;
            return true;
        }
        return false;
    }
