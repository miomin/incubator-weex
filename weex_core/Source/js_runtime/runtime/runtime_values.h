...
class RuntimeValues {
 public:
  ...
  explicit RuntimeValues(const std::string& in_string) noexcept;
  ...
  //json
  explicit RuntimeValues(const char* in_utf_8_json_string);
  ...
  static ScopeValues MakeObjectFromJsonStr(const char * utf_8_json_str) {
    return ScopeValues(new RuntimeValues(utf_8_json_str));
  }
