...
//    LOGE("NativeTimerTask is running");
    if (!WeexEnv::getEnv()->isUseRunTimeApi()){
        ...
        runtime->exeTimerFunctionForRunTimeApi(instanceId,timerFunction,global_object_v2());
        if (!repeatTimer)
            runtime->removeTimerFunctionForRunTimeApi(timerFunction,global_object_v2());
    }
