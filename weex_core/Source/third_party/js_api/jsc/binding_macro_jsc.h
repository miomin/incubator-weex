...
#include "third_party/js_api/base.h"
...
#define DECLARE_CLASS_REGISTER_OP()                                        \
static JSClassRef s_jsclass_;                                              \
...
static JSObjectRef CONSTRUCTOR_CALLBACK_FUNCTION(class_)(                  \
  ...
  auto thiz_value = RuntimeValues::MakeCommonObject(                       \
                                        static_cast<void*>(thiz_object),   \
                                        nullptr);                          \
  class_* t =  class_::JSCreate(std::move(thiz_value), arguments);         \
  JSObjectRef ob = JSObjectMake(ctx, class_::s_jsclass_,                   \
                                static_cast<void*>(t));                    \
  return ob;                                                               \
}
...
JSClassRef class_name_::s_jsclass_ = nullptr;                              \
JSClassRef class_name_::CreateClassRef(unicorn::EngineContext* context) {  \
  if (class_name_::s_jsclass_)                                             \
    return class_name_::s_jsclass_;                                        \
  JSContextRef ctx = static_cast<JSContextRef>(                            \
                                              context->GetContext());      \
...
#define REGISTER_PARENT_CLASS(parent_name_)                                \
  parent_ref = parent_name_::s_jsclass_;
